...
public class AI2 implements IPlayer {
    ...
    public AI2(boolean isP1,int maxDepth){
        this.isP1 = isP1;
    ...
    public int play(Board board){
        ...
        for(int i = 0; i<board.currentSize/2;i++){
            ...
            int result = minMaxValue(playNext,this.isP1,1,playNext.scoreP1-playNext.scoreP2);
            if(index==-1){
                maxResult = result;
                index = i;
            }
    ...
    public int minMaxValue(Board board,boolean isP1, int depth,int oldValue){
        ...
        for(int i = 0; i<sizeArray;i++){
            if(board.correctMove(convertIndex(i))){
                ...
                scoreByIndex[i] = minMaxValue(nextPos,!isP1,depth+1,board.scoreP1-board.scoreP2);
                
                pool.pushBoard(nextPos);
                if(isP1 && scoreByIndex[i] >= oldValue || !isP1 && scoreByIndex[i] <= oldValue) {
