...
MinMaxCut::MinMaxCut(bool isP1, int maxDepth) {
    this->isP1 = isP1;
    this->maxDepth = maxDepth;
    this->pool = PoolBoard(maxDepth + 5);
}
int MinMaxCut::play(Board* board) {
    std::clock_t start;
    start = std::clock();
...
int MinMaxCut::minMaxValue(Board* board, bool rushMax, int depth,int alpha, int beta) {
...
int MinMaxCut::evaluation(Board* board) {
...
int MinMaxCut::convertIndex(bool isP1, int i) {
    if (isP1) {
        return i * 2;
    }
    else {
